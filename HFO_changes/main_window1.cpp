    ...
    , M_last_connected_host( "127.0.0.1" )
{
    ...
    // Initialize image socket and server (Orr) THIS IS THE TCP SOCKET OPTION
# ifdef TCP_IMAGE_SEND
    img_socket[0] = img_socket[1] = NULL;
    img_sock_ind = 0;
    int portno = Options::instance().imagePort();
    bool server_res = img_server.listen(QHostAddress::LocalHost, portno);
    if (server_res)
        std::cout << "Image server listening on localhost port " << portno <<  std::endl;
    else {
        std::string errstring = img_server.errorString().toUtf8().constData();
        std::cout << "Image server listen failed: " << errstring << std::endl;
   }
# endif
   // image udp socket initizlization
   img_udp_socket = new QUdpSocket();
}
...
MainWindow::init()
{
    ...
    connect( M_debug_message_window, SIGNAL( configured() ),
             this, SIGNAL( viewUpdated() ) );
    connect( M_debug_message_window, SIGNAL( selectCycle( int ) ),
...
MainWindow::createActionsViewConfig()
{
    ...
    {
        {
            M_select_left_agent_act[i] = new QAction( QString( "Select Left %1" ).arg( i ), this );
            M_select_left_agent_act[i]->setShortcut( Qt::Key_0 + i );
            M_select_left_agent_act[i]->setStatusTip( tr( "Select left player %1" ).arg( i ) );
            this->addAction(  M_select_left_agent_act[i] );
        }
        {
            M_select_right_agent_act[i] = new QAction( QString( "Selct Right %1" ).arg( i ), this );
#ifdef Q_WS_MAC
            M_select_right_agent_act[i]->setShortcut( Qt::META + Qt::Key_0 + i );
#else
            M_select_right_agent_act[i]->setShortcut( Qt::CTRL + Qt::Key_0 + i );
#endif
            M_select_right_agent_act[i]->setStatusTip( tr( "Select right player %1" ).arg( i ) );
            this->addAction( M_select_right_agent_act[i] );
        }
    }
    // number 11
...
#ifdef Q_WS_MAC
    M_select_right_agent_act[10]->setShortcut( Qt::META + Qt::Key_Equal );
#else
    M_select_right_agent_act[10]->setShortcut( Qt::CTRL + Qt::Key_Equal );
#endif
...
MainWindow::startDebugServer()
{
    ...
    M_debug_server = new DebugServer( this, M_main_data.getViewHolder(), port );
...
# ifdef TCP_IMAGE_SEND
    ...
    // Initialization of TCP socket for sending images (Orr)
    while (img_server.hasPendingConnections()) {
        img_socket[img_sock_ind] = img_server.nextPendingConnection();
        img_sock_ind++;
    }

    // The following section sends out the image: (Orr)
    if (img_socket[0] || img_socket[1]) {
        std::vector<QTcpSocket*> ready_sockets;
        for (int i = 0; i < img_sock_ind; i++) {
            ready_sockets.push_back(img_socket[i]);
        }
        QImage original(M_field_canvas->size(), QImage::Format_RGB888);
        QPainter painter(&original);
        M_field_canvas->draw(painter);

        // THESE lines (and the ones commented out below)  will allow sending the image
        // as actual png, as opposed to byte array being sent right now.
        //QImageWriter img_writer;
        //img_writer.setDevice(img_socket);
        //img_writer.setFormat("png");

        QImage image = original.copy(original.width() - original.height() - 1, 0, original.height(), original.height());
        QByteArray img_arr((char*)image.bits(), image.byteCount());

        for (std::vector<QTcpSocket*>::iterator img_sock = ready_sockets.begin(); img_sock != ready_sockets.end(); img_sock++) {
            if ((*img_sock)->state() == QTcpSocket::ConnectedState) {
                //uint32_t imsize = image.byteCount();
                //std::cout << "Sending image of size: " << image.height() << " X " << image.width();
                //std::cout << " X " << image.depth() << std::endl;
                //std::cout << "In total: " << imsize << " bytes" << std::endl;
                //bool wr_success = img_writer.write(image);
                int wr_success = (*img_sock)->write(img_arr, image.byteCount());
                (*img_sock)->waitForBytesWritten(-1);
                if (wr_success > 0) {
                  //std::cout << wr_success << std::endl;
                  //std::cout << "bytes to write: " << img_socket->bytesToWrite() << std::endl;
                  //  std::cout << "success sending image" << std::endl;
                }
                else {
                    //std::string msg = img_writer.errorString().toUtf8().constData();
                    //std::cout << msg << std::endl;
                    std::cout << "Error sending image" << std::endl;
                }
            }
            else
               std::cout << "Socket exists, but not connected!" << std::endl;
        }
    }
    else
        std::cout << "Socket doesn't exist yet" << std::endl;
...
# else //send with udp
        ...
        for (int pos = 0; pos < image.byteCount(); pos += _MAX_UDP_DGRAM_BYTES) {
            qint64 send_size = (pos + _MAX_UDP_DGRAM_BYTES <= image.byteCount()) ? _MAX_UDP_DGRAM_BYTES : image.byteCount() - pos;
            int wr_success = img_udp_socket->writeDatagram(img_arr.data() + pos, send_size, QHostAddress::LocalHost, Options::instance().imagePort());
            //img_udp_socket->waitForBytesWritten(-1);
            if (wr_success < 0) {
                std::cout << img_udp_socket->errorString().toUtf8().constData() << std::endl;
            }
        }

# endif

    /* This section is to save the image to file indstead of sending it on port (Orr)

    QString file_path("./out-images/image");
    file_path += QString("%1").arg(image_save_counter, 5, 10, QChar('0'));
    file_path += QString(".png");
    QString format("png");
    if (! image.save(file_path, format.toAscii())) {
        std::cout << "Could not save image" << std::endl;
    }
    else {
        std::cout << "Saving image " << image_save_counter << std::endl;
        image_save_counter++;
    }
    */
    // end send image section
