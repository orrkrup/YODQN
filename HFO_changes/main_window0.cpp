...
# ifdef TCP_IMAGE_SEND
    ...
    if (img_socket == NULL && img_server.hasPendingConnections()) {
        img_socket = img_server.nextPendingConnection();
    }
    ...
    if (img_socket != NULL) {
        ...
        if (img_socket->state() == QTcpSocket::ConnectedState) {
            ...
            int wr_success = img_socket->write(img_arr, image.byteCount());
            img_socket->waitForBytesWritten(-1);
            if (wr_success > 0) {
...
# else //send with udp
        ...
        for (int pos = 0; pos < image.byteCount(); pos += _MAX_UDP_DGRAM_BYTES) {
            int wr_success = img_udp_socket->writeDatagram(img_arr.mid(pos, _MAX_UDP_DGRAM_BYTES), QHostAddress::LocalHost, Options::instance().imagePort());
